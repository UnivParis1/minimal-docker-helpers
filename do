...
sub check_updates_using_package_manager {
    ...
    foreach my $image (sort keys %images) {
        my $e = $images{$image};
        my $apps = join(",", @{$e->{apps}});
        log_("${GRAY}Checking updates using package manager in image $image${NC} (used by $apps)");
...
sub check_image_updates {
    ...
    foreach my $image (sort keys %images) {
        ...
        my ($new) = `/opt/dockers/.helpers/get-image-digest-from-docker.io-registry $repo $tag` =~ /(\S+)/;
