...
export CATALINA_OPTS="-Dhttp_port=$port -DmaxPostSize=$maxPostSize -DmaxParameterCount=$maxParameterCount -Dmanager_password='$manager_password' -DmaxActiveSessionsGoal='$maxActiveSessionsGoal'  -DremoteIpInternalProxies='$remoteIpInternalProxies' -Dorg.apache.catalina.session.StandardSession.ACTIVITY_CHECK=true"
